...
export class PadletStoreService {
  ...
  //Da kommt ein Array (Datenstrom) mit Padlets zurück
  ...
  //Functions für das Eintrags-Formular
  getAllEintrags() : Observable<Array<Eintrag>>{
    return this.http.get<Array<Eintrag>>(`${this.api}/eintrags`)
      .pipe(retry(3)).pipe(catchError(this.errorHandler))
  }

  getSingleEintrag(id:number) : Observable<Eintrag>{
    return this.http.get<Eintrag>(`${this.api}/eintrags/${id}`)
      .pipe(retry(3)).pipe(catchError(this.errorHandler))
  }

  createEintrag(eintrag: Eintrag) : Observable<any>{
    return this.http.post(`${this.api}/eintrags`,eintrag)
      .pipe(retry(3)).pipe(catchError(this.errorHandler))
  }

  updateEintrag(eintrag: Eintrag) : Observable<any>{
    return this.http.put(`${this.api}/eintragbearbeiten/${eintrag.id}`,eintrag)
      .pipe(retry(3)).pipe(catchError(this.errorHandler))
  }

  removeEintrag(id:number) : Observable<any>{
    return this.http.delete(`${this.api}/eintrags/${id}`)
      .pipe(retry(3)).pipe(catchError(this.errorHandler))
  }
